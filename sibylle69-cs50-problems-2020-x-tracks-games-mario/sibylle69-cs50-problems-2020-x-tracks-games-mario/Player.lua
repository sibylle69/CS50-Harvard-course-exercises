...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ["climbing"] = function(dt)
            ...
            -- check for bricks as where to stop climbing down
        ...
        ["win"] = function(dt)
            ...
            -- stop and begin new level if sprite is offscreen
            if self.x >= self.map.mapWidth then 
                self.dx = 0
                -- TODO
            end
        end
...
function Player:render()
    ...
    if self.state == "win" then 
        love.graphics.print("Level complete!", self.map.mapwidth / 2, self.map.mapHeight / 2)
    end
...
-- called in walking and jumping behaviors when checking collisions
function Player:checkPoleCollision()
    -- if sprite is moving, check for colliding with any flag piece
    if self.dx > 0 then 
        if self.map:collides(self.map:tileAt(self.x + self.width - 1, self.y)) then 
            -- if so, induce climbing and victory
            if self.map:getTile(self.x + self.width - 1, self.y) == 8 or
               self.map:getTile(self.x + self.width - 1, self.y) == 12 or
               self.map:getTile(self.x + self.width - 1, self.y) == 16 then 
                    self.state = "climbing"
                    self.dx = 0
                    self.dy = WALKING_SPEED
                    self.x = self.map:getTile(self.x + self.width - 1, self.y).x * self.map.tileWidth
            end
        end
    end
end